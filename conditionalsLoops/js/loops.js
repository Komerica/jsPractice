...
btn.addEventListener("click", function () {
  ...
  for (let i = 0; i < cities.length; i++) {
    ...
    if (searchName === cities[i]) {
      ...
      break;
      //If you have a conditional statement inside a loop that tests for a boolean value … and if that value evaluates to false, you need to exit out of the loop
    } else {
      result.textContent = "The city you typed is not in the list.";
    }
  }
});
...
for (let i = 1; i <= num; i++) {
  ...
  if (Math.floor(sqRoot) !== sqRoot) {
    ...
    continue;
    //if it's not a square root, continue, which means go back to for loop and continue by incrementing +1
  }
